"""
Command-line interface for qnap-cli.
"""
...
def find_native_binary() -> str:
    ...
    target_binary = project_root / "target" / "release" / "qnap"
    if target_binary.exists() and not target_binary.is_dir():
        return str(target_binary)

    if sys.platform == "win32":
        target_binary = project_root / "target" / "release" / "qnap.exe"
        if target_binary.exists() and not target_binary.is_dir():
            return str(target_binary)

    raise FileNotFoundError(
        "Could not find the native qnap binary. "
        "Please ensure it was built with 'cargo build --release'."
    )


def main() -> int:
    """Run the qnap command line tool."""
    try:
